...
class CRaj implements ISingleton {
	...
	protected function __construct() {
		...
		// include the site specific config.php and create a ref to $raj to be used by config.php
		$raj = &$this;
  ...
  /**
	 ...
	 * @return CRaj The instance of this class.
	 */
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= RAJ_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = RAJ_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $raj->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $raj
    ...
    include(RAJ_INSTALL_PATH . '/themes/functions.php');
  ...
  /**
   * Draw HTML for a menu defined in $raj->config['menus'].
   *
